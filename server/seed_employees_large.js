...
const firstNames = [
    ...
    "Aditi", "Abhishek", "Bhavana", "Chetan", "Deepika", "Eswar", "Firoz", "Gautam", "Harini", "Irfan",
    "Jyoti", "Kishore", "Leila", "Murali", "Navya", "Pratima", "Ravi", "Sneha", "Tarun", "Usha"
];
...
const lastNames = [
    ...
    "Kaur", "Malhotra", "Mehra", "Jain", "Agarwal", "Shetty", "Pande", "Roy", "Banerjee", "Chatterjee"
];
...
function generateEmpId(index) {
    ...
    return `EMP-${year}-S-${padIndex}`; // S for Seeding to avoid collision
}
...
async function seed() {
    ...
    for (let i = 1; i <= totalCount; i++) {
        ...
        // Target: 10% (60) high level (<7), 90% (540) low level (>=7)
        ...
        const joinDate = new Date(Date.now() - Math.floor(Math.random() * 1000 * 60 * 60 * 24 * 365 * 3));

        targets.push([
            empId,
            `QR-S-${i}-${Math.floor(Math.random() * 100000)}`,
            `TK-S-${i}-${Math.floor(Math.random() * 100000)}`,
            name,
            dept.id,
            selectedDesig.id,
            1,
    ...
    console.log(`Ready to insert ${targets.length} records. High Level (<7): ${highLevelCount}, Low Level (>=7): ${lowLevelCount}`);
